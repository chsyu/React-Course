...
export const setPage = async (dispatch, url, title, hasProducts) => {
  let products = [];
  dispatch({ type: BEGIN_PRODUCTS_REQUEST });
  dispatch({
    type: SET_PAGE_TITLE,
  ...
  try {
    if(title == 'NORDIC NEST Shopping Cart' && !hasProducts){
      dispatch({type: SET_HAS_PRODUCTS});
    ...
    } else if (title !='NORDIC NEST Shopping Cart') {
      products = await getProducts(url);
      dispatch({
        type: SET_PAGE_CONTENT,
        payload: { title, products },
